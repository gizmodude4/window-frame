...
export const getShaderInfo = (now, sunInfo) => {
    let mix = getFilterIndexAndMix(now, sunInfo);
    if (!mix) {
        return {
            foreground: {
...
function getFilterIndexAndMix(now, sunInfo) {
    if (!sunInfo) {
        return null
    }
    if (!dayNightShaderColors || !cloudShaderColors || !skyShaderColors) {
...
function createDayNightMap(sunInfo) {
    return [
        [fromMilitaryTime("00:00"), 30, 120, 225, 0.6, 1.0, -0.2, 0.8, 0.68], // midnight
        [fromMilitaryTime("04:00"), 40, 125, 215, 0.65, 0.9, -0.2, 0.7, 0.65], // late night
...
function createDayNightMapClouds(sunInfo) {
    return [
        [fromMilitaryTime("00:00"), 30, 120, 225, 0.6, 1.0, -0.4, 0.8, 0.68], // midnight
        [fromMilitaryTime("04:00"), 40, 125, 215, 0.65, 0.9, -0.4, 0.7, 0.65], // late night
...
function createDayNightMapSky(sunInfo) {
    return [
        [fromMilitaryTime("00:00"), 0, 1, 26, 4, 7, 48], // midnight
        [fromMilitaryTime("04:00"), 0, 1, 18, 4, 7, 48], // late night
...
function fromMilitaryTime(time) {
    let now = new Date(Date.now());
    let split = time.split(":")
    now.setHours(split[0], split[1], 0, 0);
    return now;
}